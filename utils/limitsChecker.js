...
const MAX_SINGLE_TRANSFER = 50000;
...
exports.checkTransferLimits = async (userId, amount) => {
  if (amount < MIN_TRANSFER || amount > MAX_SINGLE_TRANSFER) {
    throw new Error(
      `Transfer amount must be between ${MIN_TRANSFER} and ${MAX_SINGLE_TRANSFER}`,
    );
  }
  ...
  if (limits.transfers_count >= MAX_DAILY_TRANSFERS) {
    throw new Error(
      `Daily transfer limit reached (${MAX_DAILY_TRANSFERS}/day)`,
    );
  }

  if (limits.total_amount + amount > DAILY_TRANSFER_LIMIT) {
    throw new Error(
      `Daily total limit exceeded. Current: ${limits.total_amount}/${DAILY_TRANSFER_LIMIT}`,
    );
  }
