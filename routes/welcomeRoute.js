...
const { generateAndSaveWelcomeLetter } = require("../utils/welcomeService");
...
router.post("/generate", auth, async (req, res) => {
  try {
    ...
    const userRes = await db.query(
      ...
      u.business_level, 
      u.created_at,
      l.level_name FROM users u left join level_commissions l on l.level_no = u.business_level WHERE u.id = $1`,
      [userId],
    );
    ...
    const result = await generateAndSaveWelcomeLetter({
